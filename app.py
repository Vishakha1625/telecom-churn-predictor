...
/* ── Force dark scheme globally: beats system light-mode ── */
...
/* ── Header: fixed height so <<< / >>> button is never clipped ── */
...
[data-testid="stHeader"] svg{{
  fill:{TEXT}!important;display:block!important;
  visibility:visible!important;opacity:1!important;
}}
...
[data-testid="stSidebarCollapseButton"] svg{{
  ...
  opacity:1!important;width:1.3rem!important;height:1.3rem!important;
}}
...
.stTabs [data-baseweb="tab-list"]{{
  ...
  border-radius:12px;padding:4px;gap:2px;margin-bottom:24px;
}}
.stTabs [data-baseweb="tab"]{{
  background:transparent!important;color:{MUTED}!important;
  border-radius:9px!important;padding:9px 24px!important;
...
.stTabs [aria-selected="true"]{{
  ...
  box-shadow:0 2px 12px {BLUE}30!important;
}}

/* ── Selectbox / Dropdowns ── */
div[data-baseweb="select"]>div,
div[data-baseweb="select"]>div:focus-within{{
  background:{CARD2}!important;border:1px solid {BDR}!important;
...
div[data-baseweb="popover"],
div[data-baseweb="popover"]>div{{
  background:{CARD2}!important;border:1px solid {BDR2}!important;
  border-radius:10px!important;
}}
div[data-baseweb="popover"] li,
div[data-baseweb="popover"] [role="option"]{{
...
button[aria-label="Decrement"]{{
  background:{CARD2}!important;border:1px solid {BDR}!important;
  border-radius:6px!important;color:{TEXT}!important;fill:{TEXT}!important;
}}
...
label,[data-testid="stWidgetLabel"],
[data-testid="stWidgetLabel"] p{{
  color:{SUB}!important;-webkit-text-fill-color:{SUB}!important;
}}
...
/* ── Prediction page button — force dark bg even in light system mode ── */
div.stButton>button,
...
[data-testid="baseButton-secondary"]{{
  ...
  box-shadow:0 4px 20px {BLUE}40!important;
...
button[kind="primary"]:hover{{
  ...
  background-color:#6BA3F9!important;
  box-shadow:0 8px 30px {BLUE}55!important;
  transform:translateY(-1px)!important;
}}
/* disabled state */
div.stButton>button:disabled{{
  background:{BDR}!important;color:{MUTED}!important;
...
[data-testid="stExpander"] svg{{fill:{TEXT}!important;}}
...
.stCaption p{{color:{SUB}!important;-webkit-text-fill-color:{SUB}!important;}}

/* ── Animations ── */
@keyframes pdot{{0%,100%{{opacity:1;}}50%{{opacity:.2;}}}}
.pdot{{display:inline-block;width:7px;height:7px;border-radius:50%;
  background:{GREEN};animation:pdot 2s infinite;
  box-shadow:0 0 6px {GREEN};margin-right:6px;vertical-align:middle;}}
@keyframes sup{{from{{opacity:0;transform:translateY(14px);}}to{{opacity:1;transform:translateY(0);}}}}
.sup{{animation:sup .4s cubic-bezier(.22,1,.36,1) both;}}
</style>
""", unsafe_allow_html=True)
...
# st.dataframe uses a <canvas> which ignores
# CSS — always shows the browser/system colors.
# This renders a real <table> via st.markdown
# so it always matches the dark app theme.
# ─────────────────────────────────────────────
def show_df(dataframe, hide_index=True):
    TH = (f"background:{CARD2};color:{TEXT};padding:9px 14px;"
          f"text-align:left;font-size:.79rem;font-weight:700;"
          f"border-bottom:2px solid {BDR2};white-space:nowrap;"
          f"-webkit-text-fill-color:{TEXT};letter-spacing:.3px;")
    TD  = (f"color:{TEXT};padding:8px 14px;font-size:.79rem;"
           f"border-bottom:1px solid {BDR};white-space:nowrap;"
           f"-webkit-text-fill-color:{TEXT};background:{CARD};")
    TD2 = (f"color:{TEXT};padding:8px 14px;font-size:.79rem;"
           f"border-bottom:1px solid {BDR};white-space:nowrap;"
           f"-webkit-text-fill-color:{TEXT};background:{CARD2};")
    html = (f'<div style="overflow-x:auto;border-radius:10px;'
            f'border:1px solid {BDR};background:{CARD};margin-bottom:8px;">'
            f'<table style="width:100%;border-collapse:collapse;background:{CARD};">'
            f'<thead><tr>')
    if not hide_index:
        html += f'<th style="{TH}"> </th>'
    for col in dataframe.columns:
        html += f'<th style="{TH}">{col}</th>'
    html += '</tr></thead><tbody>'
    for i, (_, row) in enumerate(dataframe.iterrows()):
        s = TD2 if i % 2 == 0 else TD
        html += '<tr>'
        if not hide_index:
            html += f'<td style="{s};color:{SUB};-webkit-text-fill-color:{SUB};">{i}</td>'
        for val in row:
            if isinstance(val, float):
                cell = f"{val:.4f}" if 0 < abs(val) < 10 else f"{val:.2f}"
            else:
                cell = str(val)
