...
/* ── Header: keep real height so toggle button is visible ── */
...
[data-testid="stSidebarCollapseButton"] svg{{
  ...
  opacity:1!important;fill:{TEXT}!important;
  color:{TEXT}!important;width:1.4rem!important;height:1.4rem!important;
}}
...
[data-testid="collapsedControl"]{{
  ...
  transform:translateY(-50%)!important;padding:10px 5px!important;
}}
[data-testid="stSidebarCollapsedControl"] button,
[data-testid="collapsedControl"] button{{
  display:flex!important;visibility:visible!important;
...
/* ── Sidebar background + ALL text/icon colours ── */
[data-testid="stSidebar"]{{
...
[data-testid="stSidebar"] a{{color:{BLUE}!important;-webkit-text-fill-color:{BLUE}!important;}}
[data-testid="stSidebar"] svg{{fill:{TEXT}!important;color:{TEXT}!important;}}
...
# st.dataframe uses a <canvas> renderer that
# ignores all CSS — so tables always show in
# the browser's default light theme. This helper
# renders a plain <table> via st.markdown which
# is fully CSS-styleable and matches the dark
# app theme on every browser & Streamlit Cloud.
# ─────────────────────────────────────────────
def show_df(dataframe, hide_index=True):
    ...
    TD = (f"color:{TEXT};padding:8px 14px;font-size:.79rem;"
          ...
          f"-webkit-text-fill-color:{TEXT};")
    TD2 = f"background:{CARD2};" + TD          # alternating row
    ...
    for i, (_, row) in enumerate(dataframe.iterrows()):
        ...
        html += '<tr>'
        if not hide_index:
            html += f'<td style="{TD};color:{SUB};-webkit-text-fill-color:{SUB};">{i}</td>'
        for val in row:
